...
export class Client {
  ...
  notifyJob(job, cleanOverride = null) {
    if (!this.subscribed || !this.alive) return;
    try { this.sock.write(job.notifyLine(this.payoutScript, cleanOverride)); } catch { this.destroy('write fail'); }
  }
  setDifficulty(d) {
  ...
  onSubscribe(id, params) {
    ...
    // diff + current job go out immediately after authorize (some firmware
    // wants authorize first); but plenty of devices subscribe-only, so push now.
    this.pushWork(true);
  }
  ...
  onAuthorize(id, params) {
    ...
    } catch (e) { payoutErr = e; /* plain worker name -> pool default payout */ }
    // Loud warning ONLY when the username clearly MEANT to be an address but is
    // invalid (typo, wrong network) — a silent fallback there would pay this
    // miner's blocks to the pool default wallet without anyone noticing.
    // Short plain worker names (e.g. "bitaxe-1") are intentional and stay quiet.
    ...
    this.log(`authorized (agent="${this.agent}" vr=${this.versionRolling ? this.versionMask.toString(16) : 'no'} payout=${this.payoutAddress})`);
  }
  ...
  onSubmit(id, params) {
    ...
    // ntime window. Lower bound is the job's own curtime — NEVER below it.
    // The node guarantees curtime > median-time-past, so this makes it
    // impossible to build a block whose timestamp the node would reject as
    // "time-too-old" (consensus MTP rule). Miners only ever roll ntime
    // FORWARD for extra nonce space, so this rejects nothing legitimate.
    // (ckpool enforces exactly this: ntime32 < wb->ntime32 => reject.)
    const now = Math.floor(nowMs() / 1000);
    if (ntime < job.curtime || ntime > now + 7000) {
      reg.recordRejected(this.worker, 'ntime'); return this.result(id, null, E.BAD_NTIME);
    }
...
export class StratumServer extends EventEmitter {
  ...
  nextExtranonce1() {
    for (;;) {
      const b = Buffer.alloc(this.cfg.extranonce1Size);
      randomBytes(this.cfg.extranonce1Size - 2).copy(b, 0);
      b.writeUInt16BE((this.enCounter = (this.enCounter + 1) & 0xffff), this.cfg.extranonce1Size - 2);
      const hex = b.toString('hex');
      if (!this.activeEn1.has(hex)) { this.activeEn1.add(hex); return hex; }
    }
  }
  ...
  broadcast(job) {
    for (const c of this.clients) c.notifyJob(job);
    this.emit('log', `job ${job.id} ${job.kind} @${job.height} txs=${job.txs.length} clean=${job.clean} -> ${this.clients.size} miners`);
  }
