...
export class Job {
  ...
  notifyParams(script, cleanOverride = null) {
    ...
    return [
      ...
      this.nbits,
      this.curtime.toString(16).padStart(8, '0'),
  ...
  notifyLine(script, cleanOverride = null) {
    const key = (script ? script.toString('hex') : 'd') + '|' + (cleanOverride === null ? 'j' : cleanOverride ? '1' : '0');
    if (!this.lineCache) this.lineCache = new Map();
    let line = this.lineCache.get(key);
    if (!line) {
      line = JSON.stringify({ id: null, method: 'mining.notify', params: this.notifyParams(script, cleanOverride) }) + '\n';
...
export class TemplateManager extends EventEmitter {
  ...
  async selfAudit() {
    ...
    const hex = job.buildBlockHex(null, Buffer.alloc(this.enTotal, 0), job.curtime, 0, job.version);
    ...
    try {
      ...
      if (nextHeight % 2016 !== 0) {
        ...
        const er = await this.rpc.proposeBlock(ej.buildBlockHex(null, Buffer.alloc(this.enTotal, 0), ej.curtime, 0, ej.version));
  ...
  async fullRefresh(forceClean) {
    ...
    try {
      ...
      // skip pointless re-notify: same tip, fresh job already out, not forced
      if (sameTip && !clean && this.current && this.current.kind !== 'empty'
          && nowMs() - this.current.createdAt < this.cfg.templateRefreshMs * 0.5) return;
