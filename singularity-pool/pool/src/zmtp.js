...
export class ZmtpSub extends EventEmitter {
  ...
  _onData(d) {
    ...
    // surface parse failures (never silent): bitcoind never sends bad frames, so
    // if this ever fires it's a real problem worth seeing in the logs.
    try { this._drain(); } catch (e) { this.emit('warn', `${this.endpoint}: ${e.message}`); this.sock.destroy(); }
  }
    ...
    while (true) {
      ...
      if (flags & 0x04) { // command
        ...
        if (this.phase === 'handshake' && name === 'READY') {
          ...
          this.alive = true;
          this.backoff = 500;
          this.sock.setTimeout(45 * 60 * 1000, () => this.sock.destroy()); // idle-reconnect watchdog
